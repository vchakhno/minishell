...
/*   Updated: 2024/01/27 23:19:21 by vchakhno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	run_redirections(
	t_vector redirs, t_backup_fds *backup, bool *recovers, t_env env, t_u8 exit_status
) {
	t_redirection	*redir;
	t_u32			i;
	...
	while (i < redirs.size)
	{
		...
		if (!run_redirection(redir, recovers, env, exit_status))
		{
			if (*recovers)
